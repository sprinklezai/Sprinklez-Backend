...
function shiftMonthYear(monthValue, offset) {
  const match = /^(\d{4})_(\d{2})$/.exec(String(monthValue || ""));
  if (!match) return "";
...
function monthlyRevenue(days, country, storeCode) {
  ...
  for (const day of days || []) {
    ...
    if (month) {
      monthMap.set(month, (monthMap.get(month) || 0) + dayRevenue);
    }
  }
...
function getRevenueComparison({
  ...
  const currentMap = monthlyRevenue(
    filterDays(currentPeriod.days || [], fromDate, toDate),
    country,
    store
  );

  const priorSelectedMonth = shiftMonthYear(selectedMonth, -1);
  const priorFromDate = shiftDateYear(fromDate, -1);
  const priorToDate = shiftDateYear(toDate, -1);
  ...
  if (priorSelectedMonth) {
    try {
      ...
      priorMap = monthlyRevenue(
        filterDays(priorPeriod.days || [], priorFromDate, priorToDate),
        country,
        store
      );
    } catch (error) {
  ...
  const monthlyRevenueComparison = Array.from(currentMap.keys())
    .sort()
    .map((monthValue) => {
      const priorMonthValue = shiftDateYear(`${monthValue}-01`, -1).slice(0, 7);

      return {
        month: monthValue,
        label: monthLabel(monthValue),
        current: Number(currentMap.get(monthValue) || 0),
        prior: Number(priorMap.get(priorMonthValue) || 0),
      };
    });

  return {
    monthlyRevenueComparison,
    priorPeriodAvailable: Array.from(priorMap.values()).some(
      (value) => Number(value || 0) !== 0
    ),
  };
}

module.exports = { getRevenueComparison };
